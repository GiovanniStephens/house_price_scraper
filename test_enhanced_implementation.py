...
class TestEnhancedScrapeHousePrices(unittest.TestCase):
    ...
    def test_enhanced_scraping_returns_scraping_result(self):
        ...
        self.mock_driver.elements = {
            '//*[@id="mat-tab-content-0-0"]/div/div[2]/div[1]/homes-hestimate-tab/div[1]/homes-price-tag-simple/div/span[2]': MockElement("$1.2M")
        }
    ...
    def test_enhanced_scraping_regex_fallback(self):
        ...
        self.mock_driver.page_source = "Property estimate is $1.8M based on recent sales data"
    ...
    def test_enhanced_scraping_propertyvalue_special_case(self):
        ...
        self.assertTrue(result.success)
        self.assertIsNone(result.prices["midpoint"])  # Should be None for external calculation
...
class TestEnhancedScrapingIntegration(unittest.TestCase):
    ...
    def test_scrape_all_house_prices_enhanced_interface(self):
        ...
        enhanced_params = ["enable_retry", "rate_limit", "validate_prices", "enable_logging"]
    ...
    def test_selector_strategy_real_application(self):
        ...
        test_cases = [
            ...
            },
            {
                ...
                "driver_setup": {"elements": {"//span[@class='price']": MockElement("$1.5M")}},
                "expected": "$1.5M"
            },
            {
                "strategy": {"type": "regex_fallback", "pattern": r"\$\d+\.?\d*M"},
                "driver_setup": {"page_source": "Property valued at $1.8M in current market"},
                "expected": "$1.8M"
            }
        ]
    ...
    def test_enhanced_regex_patterns(self):
        ...
        for html in html_samples:
            with self.subTest(html=html[:30]):
                ...
                for price in prices:
                    self.assertRegex(price, r'(\$?\d+(?:,\d{3})*(?:\.\d+)?[MK]?|\$[\d,]+(?:\.\d+)?)')
